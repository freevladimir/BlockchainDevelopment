...
contract Ownable {
    ...
    function setContract(address _address) onlyOwner{
    ...
    modifier onlyOwnerOrContract() {
        require(msg.sender == owner || msg.sender == contr);
        _;
    }
...
contract MintableToken is StandardToken, Ownable {
     ...
     */
    function mint(address _to, uint256 _amount) onlyOwnerOrContract canMint returns (bool) {
...
contract BurnableToken is MintableToken {
     ...
     */
    function burn(address _address, uint _value) onlyOwnerOrContract {
        require(_value > 0);
...
contract SimpleTokenCoin is BurnableToken {
    ...
    string public constant name = "Token Fund";

    string public constant symbol = "TKF";

    uint32 public constant decimals = 18;
    
    uint public constant coin = 10 ** uint256(decimals);
...
contract Crowdsale is Ownable {
    ...
    function changeWalletFund(address _wallet) onlyOwner{
        require(_wallet!=0x0);
        walletFund = _wallet;
    }
    
    
    function Crowdsale(SimpleTokenCoin _token, string _name, address _wallet, uint _rate) {
        token = _token;
        namefund = _name;
        walletFund = _wallet;
        rate = _rate;
    ...
    function createTokens() payable {
        if(msg.sender != owner){
            uint tokens = msg.value.mul(token.coin()).div(rate);
